...
#[test]
#[ignore = "requires the toml-config/toml-test fixture submodule"]
fn corpus_matches_reference_json() {
    let test_dir = toml_test_dir();
    assert!(
        test_dir.is_dir(),
        "missing TOML fixture corpus at {}. Run `git submodule update --init toml-config/toml-test`.",
        test_dir.display(),
    );

    let mut have_failures = false;
    let mut num = 0;
    for path in WalkDir::new(&test_dir) {
...
fn toml_test_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("toml-test/tests/valid")
}
